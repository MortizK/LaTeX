...
\begin{enumerate}
    \item Wenn A schuldig und B unschuldig, so ist C schuldig
    \item C arbeitet niemals allein
    \item A arbeitet niemals mit C
    \item Niemand außer A, B oder C war beteiligt und mindestens ein muss es gewesen sein.
\end{enumerate}

\begin{enumerate}
    \item $A\land \neq B\to C$
    \item $C\to (A\lor B)$ (mit 4.)
    \item $A\to\neg C$
    \item $A\lor B\lor C$
\end{enumerate}
