...
\begin{enumerate}
    \item[i)] $det(a_1, a_2, \dots, a_k, \dots, a_l, \dots, a_n) = -det(a_1, a_2, \dots, a_l, \dots, a_k, \dots, a_n)$ alternierend: Wenn wir zwei Werte Tauschen, können wir das, aber handeln uns ein Minus ein.
    \item[ii)] $det(t_1a_1+s_1b_1, a_2, \dots, a_n) = t_1*det(a_1, a_2, \dots, a_n) + s_1*det(b_1, a_2, \dots, a_n)$ multilinear: Dadurch, dass wir nach i) auch alles Tauschen können ist jeder Wert linear.
    \item[iii)] $det(e_1, e_2, \dots, e_n) = 1$ normiert: Die Determinante, von den allen Normalvektoren ist 1. $e_1=[1, 0, 0, \dots, 0]$ und alle weiteren sind die Kanonische Basis: $K:={e_1, e_2, \dots, e_n}$
\end{enumerate}
