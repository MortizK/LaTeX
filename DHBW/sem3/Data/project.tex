...
Die Grafik wurde ausgetauscht um den aktuellen Stand zu repräsentieren.
...
Verkauf: \{[\underline{VerkaufID: integer}, Datum: datetime]\}
...
Die SQL-Implementierung überführt die Relationen aus dem relationalen Modell insgesamt sorgfältig und vollständig in PostgreSQL. Die Struktur ist verständlich, die Löschregeln (CASCADE, RESTRICT, SET NULL) sind sinnvoll gesetzt, und die Testdaten ermöglichen aussagekräftige Abfragen. Besonders hervorzuheben ist die korrekte Trennung der Telefonnummern sowie die Implementierung der Spezialisierung Azubi mit Verantwortlichem.

Dennoch gibt es einige Punkte bei der Übertragung vom relationalen Modell zu beachten:

\begin{enumerate}
    \item \textbf{Konsistenz bei Lagerbeständen}: Das relationale Modell definiert Zutat mit einem globalen Bestand-Attribut. Die liefert-Relation verknüpft Zulieferer, Zutaten und Filialen, enthält aber keine Bestands- oder Mengenwerte. Dies ist inkonsistent: Falls jede Filiale eigene Zutatenbestände haben soll (wie in vorhanden für Produkte), sollte dies explizit durch eine zusätzliche Spalte in liefert oder eine separate Lagerbestands-Relation modelliert sein. Die aktuelle Implementierung lässt unklar, wie filialgebundene Zutatenbestände verwaltet werden.

    \item \textbf{Rezept-Mengen in beinhaltet}: Die beinhaltet-Relation verbindet Rezepte mit Zutaten, enthält aber keine Spalten für die benötigte Menge oder Einheit. Dies ist für die praktische Nutzung essentiell (z.B. ``500g Mehl für Rezept 1''). Die Relation sollte um ``Menge'' und optional ``Einheit'' ergänzt werden.

    \item \textbf{Produktzuweisung zu Rezepten pro Filiale}: Die Tabelle Produkt hat ein einzelnes RezeptID-Feld. Falls ein Produkt in verschiedenen Filialen unterschiedliche Rezepte verwenden kann, ist dies nicht abbildbar. Das relationale Modell sollte präzisieren, ob dies über eine ternäre Relation (ProduktID, FilialeID, RezeptID) zu lösen ist oder ob ein Produkt global an ein Rezept gebunden ist.
\end{enumerate}

Insgesamt ist die SQL-Implementierung qualitativ gut gelungen. Die genannten Punkte sind eher Klärungen zwischen dem relationalen Modell und der SQL-Umsetzung, um sicherzustellen, dass beide konsistent sind.
