...
\begin{flushright}
von Niklas Reusch für Moritz Köhler am 03.06.2026
\end{flushright}

Das View-Skript ist allgemein sehr ausführlich und deckt die Anforderungen größtenteils ab. Ich finde sehr gut, dass für die unterschiedlichen Benutzerrollen sehr praxisnahe Views angelegt worden sind, wie zum Beispiel filialbezogene Informationen und Produktdaten. Außerdem werden in vielen Views einzelne Tabellen durch Joins zusammengeführt, wodurch die nötigen Informationen enthalten und gut verständlich sind.

Verbessern könnte man die \lstinline|View v_mitarbeiter_kontakt|, da die Zuordnung eines Mitarbeiters zu einer Filiale über die Schichten mit der Tabelle \lstinline|durchgefuehrt_von| erfolgt. Das Problem hierbei ist, dass unser SQL-Schema die Beziehung von einem Mitarbeiter zu mehreren Schichten erlaubt, wodurch Duplikate vorkommen können. Als Lösung könnte man hier zum Beispiel mit dem \lstinline|DISTINCT| Statement arbeiten. 

\begin{sloppypar}
Dasselbe gilt für die \lstinline|Views v_filialleiter_dashboard| und \lstinline|v_eigentuemer_uebersicht|, da durch die Duplikate ein Mitarbeiter möglicherweise mehrmals gezählt werden kann.
Hierfür könnte man statt \lstinline|COUNT(*)| besser \lstinline|COUNT(DISTINCT m.MitarbeiterID)| benutzen.
\end{sloppypar}

\begin{sloppypar}
Zudem wird in der \lstinline|View v_verkaufsstatistik_produkt| bei der Berechnung nicht das Mengenattribut der Produkte berücksichtigt. In der \lstinline|View v_verkaufsstatistik_produkt_filiale| machst du das mit \lstinline|SUM(vk.Menge) * p.Preis| bereits richtig.
\end{sloppypar}

Der Rest ist aber sehr logisch und gelungen.
